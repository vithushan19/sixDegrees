...
public class ApiRequester {
	...
	public List<String> getMoviesForActor(String actor) {
		...
		for (int i = 1; i < actor.length(); i++) {
			if (Character.isUpperCase(actor.charAt(i))
					&& !Character.isWhitespace(actor.charAt(i - 1))) {
				if (actor.charAt(i - 1) == 'c') {// possibly a McSomething
					continue;
				}
				actor = actor.substring(0, i) + " "
						+ actor.substring(i, actor.length());
				i++;
			}
		}
	...
	public String getMovieId(String movie) {
		...
		JSONObject jResponse = makeRequest(BASE_URL + "movies.json?apikey="
				+ API_KEY + "&q=" + movie + "&page_limit=1");
		try {
			String selfUrl = jResponse.getJSONArray("movies").getJSONObject(0)
					.getJSONObject("links").getString("self");
			int movieIdIndex = selfUrl.indexOf("movies/") + 6;
			return selfUrl.substring(movieIdIndex,
					selfUrl.length() - ".json".length());
	...
	public List<String> getActorsForMovies(String movie) {
		...
		JSONObject jResponse = makeRequest(BASE_URL + "movies/" + movieId
	...
	private static String convertStreamToString(InputStream is) {
		 ...
		 * appended to a StringBuilder and returned as String.l
		 */
